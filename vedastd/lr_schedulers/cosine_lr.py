...
class CosineLR(_Iter_LRScheduler):
    ...
    def __init__(self, optimizer, niter_per_epoch, max_epochs, last_iter=-1,
                 warmup_epochs=0, iter_based=True):
    ...
    def get_lr(self):
        if self.last_iter < self.warmup_iters:
            multiplier = 0.5 * (1 - cos(pi * (self.last_iter /
                                              float(self.warmup_iters))))
        else:
            multiplier = 0.5 * (1 + cos(pi * ((self.last_iter - self.warmup_iters)
