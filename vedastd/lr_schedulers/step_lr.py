...
class StepLR(_Iter_LRScheduler):
    def __init__(self, optimizer, niter_per_epoch, max_epochs, milestones,
                 gamma=0.1, last_iter=-1, warmup_epochs=0, iter_based=True):
        self.max_iters = niter_per_epoch * max_epochs
        self.milestones = milestones
        self.count = 0
        self.gamma = gamma
        self.warmup_iters = int(niter_per_epoch * warmup_epochs)
        super(StepLR, self).__init__(optimizer, niter_per_epoch,
