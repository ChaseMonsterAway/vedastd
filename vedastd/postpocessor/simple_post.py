...
class PsePostprocessor:
    def __init__(self, thresh=1.0, min_kernel_area=5, min_score=0.93, box_thresh=0.7, max_candidates=100, unclip_ratio=1.5,
                 resize=False, name=('pred_text_map', 'pred_kernels_map'), min_size=3, debug=False):
    ...
    def boxes_from_bitmap(self, kernels, score, text, scale, h, w):
        '''
        kernels: map with shape (7, H, W),
            whose values are binarized as {0, 1}
        '''
        assert kernels.size(0) == 7
